...
const chFetcher = async (url: string) => {
  const res = await fetch(url);
  const data = await res.json();
  const listItems = data.items.reduce(
    (acc: Array<Object>, cur: Object, idx: number) => {
      const obj = {
        key: idx,
        id: cur.id || "",
...
const chFilter = (data: [], query: {}) => {
  ...
  if (query.minSubscriberCount) {
    filteredData = filteredData.filter(
      (item) => item.subscriberCount > parseInt(query.minSubscriberCount)
    );
  }
  if (query.maxSubscriberCount) {
    filteredData = filteredData.filter(
      (item) => item.subscriberCount < parseInt(query.maxSubscriberCount)
    );
  }
  if (query.minViewCount) {
    filteredData = filteredData.filter(
      (item) => item.viewCount > parseInt(query.minViewCount)
    );
  }
  if (query.maxViewCount) {
    filteredData = filteredData.filter(
      (item) => item.viewCount < parseInt(query.maxViewCount)
    );
  }
  return filteredData;
};
